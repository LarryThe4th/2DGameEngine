...
void Agent::draw(GameEngine::SpriteBatch& _spriteBatch) {
    ...
    //static int textureID = GameEngine::ResourceManager::GetTexture("Textures/Shooter.png").id;
    ...
    //_spriteBatch.AddSprite(destRect, uvRect, textureID, 0.0f, _color);
}

bool Agent::applyDamage(float damage) {
  
    _health -= damage;
    // If the player died, return true
    if (_health <= 0) {
        return true;
    }
    return false;
...
void Agent::checkTilePosition(const std::vector<std::string>& levelData,
    ...
    if (levelData[(unsigned int)gridPos.y][(unsigned int)gridPos.x] != '.') {
